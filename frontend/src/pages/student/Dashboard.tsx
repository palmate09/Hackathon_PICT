...
import { Application, AIRecommendation, JobOpportunityCard } from '../../types';
...
const StudentDashboard: React.FC = () => {
  ...
  const handleApply = (rec: AIRecommendation) => {
    // Convert recommendation to JobOpportunityCard format
    const opp: JobOpportunityCard = {
      id: rec.opportunity.id,
      title: rec.opportunity.title,
      company_name: rec.opportunity.company_name || '',
      location: rec.opportunity.location || 'Remote',
      duration: rec.opportunity.duration || 'N/A',
      tags: rec.matched_skills || [],
      eligible: true,
      match: rec.score,
      status: 'eligible',
      applied: Boolean(rec.opportunity.has_applied),
      match_data: {
        match_percentage: rec.score,
  ...
  const handleViewDetails = (rec: AIRecommendation) => {
    navigate('/student/opportunities', { state: { opportunityId: rec.opportunity.id } });
  };
            ...
            <div key={rec.opportunity.id} className="card" style={styles.oppCard}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
                <button 
                  ...
                  style={{ flex: 1, textAlign: 'center', fontSize: '0.8125rem', padding: '0.5rem 0.75rem' }}
                >
                  View Details
                </button>
                <button 
