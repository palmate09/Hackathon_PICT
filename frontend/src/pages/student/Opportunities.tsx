...
const Opportunities: React.FC = () => {
  ...
  const routeOpportunityId = useMemo(() => {
    const value = Number(params.opportunityId);
  ...
  const stateOpportunityId = useMemo(() => {
    const locationState = location.state as { opportunityId?: unknown } | null;
    const value = Number(locationState?.opportunityId);
    if (!Number.isFinite(value) || value <= 0) {
      return null;
    }
    return value;
  ...
  useEffect(() => {
    ...
    if (handledDeepLinkRef.current === targetOpportunityId) {
      return;
    }
    if (loading) {
      return;
    ...
    const matched = matchedOpportunities.find((opp) => Number(opp.id) === targetOpportunityId);
    const fromSummary = summary?.opportunities?.find(
      (opp) => Number(opp.id) === targetOpportunityId
    );

    const target = matched || fromSummary;
    if (!target) {
