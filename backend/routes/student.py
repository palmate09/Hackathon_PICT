...
def get_full_profile_data():
    ...
    if profile.first_name: completion_score += 5
    if profile.last_name: completion_score += 5
    total_fields += 10
    if profile.phone: completion_score += 5
    if profile.date_of_birth: completion_score += 3
    if profile.address: completion_score += 2
    total_fields += 10
    if profile.course: completion_score += 5
    if profile.specialization: completion_score += 5
    total_fields += 10
    ...
    # Skills (15 points)
    student_skills = StudentSkill.query.filter_by(student_id=profile.id).all()
    if student_skills and len(student_skills) > 0:
        completion_score += 15
    total_fields += 15
    
    # Internships (15 points) - mandatory
    if sections.get('internships') and len(sections['internships']) > 0:
        completion_score += 15
    total_fields += 15
    ...
    additional_sections = ['projects', 'experiences', 'certifications', 'trainings']
    for section in additional_sections:
        if sections.get(section) and len(sections[section]) > 0:
            completion_score += 2.5
    total_fields += 10
    
    completion_percentage = round((completion_score / total_fields) * 100) if total_fields > 0 else 0
...
def attachments_collection():
    ...
    if 'file' in request.files:
        ...
        attachments_dir = os.path.join('public', 'attachments')
...
def upload_resume():
    try:
        ...
        # Store in local public filesystem
        resumes_dir = os.path.join('public', 'resumes')
