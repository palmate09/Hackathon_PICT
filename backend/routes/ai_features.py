...
from models import db, StudentProfile, Opportunity, Application, User, StudentSkill
...
def calculate_skill_match(student_skills, required_skills):
    ...
    # Fuzzy matching for partial matches (e.g., "python" matches "python programming")
    remaining_required = set(required_skills_lower) - set(student_skills_lower)
    remaining_student = set(student_skills_lower) - set(required_skills_lower)
    
    fuzzy_matches = 0
    for req_skill in remaining_required:
        for stud_skill in remaining_student:
            # Check if one contains the other (fuzzy match)
            if req_skill in stud_skill or stud_skill in req_skill:
                fuzzy_matches += 1
                remaining_student.discard(stud_skill)  # Don't match same skill twice
                break
    
    total_matches = matched + fuzzy_matches
    return (total_matches / len(required_skills)) * 100 if required_skills else 0.0
...
def get_recommendations():
    try:
        ...
        # Get student skills from StudentSkill relationship (preferred) or fallback to JSON
        student_skill_rels = StudentSkill.query.filter_by(student_id=profile.id).all()
        if student_skill_rels:
            student_skills = [ss.skill.name for ss in student_skill_rels if ss.skill]
        else:
        ...
        for opp in opportunities:
            ...
            # Use SkillsMatchingService for accurate matching
            try:
                match_result = SkillsMatchingService.calculate_match_score(profile.id, opp.id)
                skill_match = match_result.get('match_percentage', 0.0)
                ...
                matched_skills_list = list(set([s.lower() for s in student_skills]) & set([s.lower() for s in required_skills]))
