...
class SkillsMatchingService:
    ...
    def update_student_skills(student_id: int, skill_names: List[str], 
        ...
        """Update student skills from list of skill names (idempotent upsert)."""
        proficiency_levels = proficiency_levels or {}
        allowed_levels = {"beginner", "intermediate", "advanced", "expert"}

        # Normalize incoming proficiency map once so lookups are stable.
        normalized_proficiency = {}
        for key, level in proficiency_levels.items():
            if not key:
                continue
            normalized_key = SkillsMatchingService.normalize_skill_name(str(key))
            normalized_level = str(level or "intermediate").strip().lower()
            if normalized_level not in allowed_levels:
                normalized_level = "intermediate"
            normalized_proficiency[normalized_key] = normalized_level

        # Step 1: de-duplicate requested skills by normalized name first.
        # This prevents duplicate inserts and avoids query-triggered autoflush conflicts.
        deduped_input: List[str] = []
        seen_normalized_names = set()
        for raw_name in skill_names or []:
            cleaned = str(raw_name or "").strip()
            if not cleaned:
                continue
            normalized = SkillsMatchingService.normalize_skill_name(cleaned)
            if not normalized or normalized in seen_normalized_names:
                continue
            seen_normalized_names.add(normalized)
            deduped_input.append(cleaned)

        # Step 2: resolve to unique skill IDs and desired proficiency.
        desired_by_skill_id: Dict[int, Dict[str, str]] = {}
        for skill_name in deduped_input:
            skill = SkillsMatchingService.get_or_create_skill(skill_name)
            if not skill:
                continue

            normalized = SkillsMatchingService.normalize_skill_name(skill_name)
            proficiency = normalized_proficiency.get(normalized, "intermediate")

            desired_by_skill_id[skill.id] = {
                "name": skill_name,
                "proficiency": proficiency,
            }

        # Step 3: upsert rows instead of delete+insert.
        # This is robust against duplicate input and minimizes writes.
        existing_rows = StudentSkill.query.filter_by(student_id=student_id).all()
        existing_by_skill_id = {row.skill_id: row for row in existing_rows}
        desired_skill_ids = set(desired_by_skill_id.keys())

        # Remove stale skills no longer requested.
        for row in existing_rows:
            if row.skill_id not in desired_skill_ids:
                db.session.delete(row)

        # Insert/update desired skills.
        result_rows: List[StudentSkill] = []
        for skill_id, meta in desired_by_skill_id.items():
            existing_row = existing_by_skill_id.get(skill_id)
            if existing_row:
                existing_row.proficiency_level = meta["proficiency"]
                result_rows.append(existing_row)
            else:
                new_row = StudentSkill(
                    student_id=student_id,
                    skill_id=skill_id,
                    proficiency_level=meta["proficiency"],
                )
